...
public class Chase : Node
{
    ...
    public override void Execute()
    {
        ...
        Vector3 targetDir = BT._player.transform.position - BT.transform.position;
        Vector3 dir = (BT._player.transform.position - BT.transform.position).normalized;


        Physics.Raycast(BT.transform.position, targetDir.normalized, out BT.hit);

        Debug.DrawRay(BT.transform.position, BT._player.transform.position - BT.transform.position, Color.red);
        ...
        if (Vector3.Distance(BT.transform.position, BT._player.transform.position) < BT.maxdistance || BT.angle <= BT.vision && BT.hitInfo.transform == BT._player.transform || BT.ect.health < BT.ect.MaxHealth)
        {
            ...
            if (Physics.Raycast(BT.transform.position, BT.transform.forward, out BT.hitInfo, raycastlenght))
            {
                ...
                if (BT.hitInfo.transform != BT._player.transform)
                {
